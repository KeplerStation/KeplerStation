		...
		if("MessageCentCom")
			if(authenticated==2)
				if(!checkCCcooldown())
					to_chat(usr, "<span class='warning'>Arrays recycling.  Please stand by.</span>")
					return
				var/input = stripped_input(usr, "Please choose a message to transmit to Head Office via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "Send a message to Head Office.", "")
				if(!input || !(usr in view(1,src)) || !checkCCcooldown())
					return
				playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
				CentCom_announce(input, usr)
				to_chat(usr, "<span class='notice'>Message transmitted to Head Office.</span>")
				usr.log_talk(input, LOG_SAY, tag="Head Office announcement")
				deadchat_broadcast("<span class='deadsay'><span class='name'>[usr.real_name]</span> has messaged Head Office, \"[input]\" at <span class='name'>[get_area_name(usr, TRUE)]</span>.</span>", usr)
				CM.lastTimeUsed = world.time
		...
		if(STATE_DEFAULT)
			if (authenticated)
				...
				if (authenticated==2)
					...
					if(!(obj_flags & EMAGGED))
						dat += "<BR>\[ <A HREF='?src=[REF(src)];operation=MessageCentCom'>Send Message to Head Office</A> \]"
					else
