	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			if(0)
				...
				dat += "<BR><FONT SIZE=1>Property of Horizons</FONT>"
				if(GLOB.news_network.wanted_issue.active)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a Head Office Director. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by a Head Office Director. You cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a Head Office Director. You cannot place a D-Notice upon it.","Ok")
				return
...
/obj/item/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard many Space Stations."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>99% recycled plastic newspaper. I wish we had actual paper.</FONT></div><HR>"
