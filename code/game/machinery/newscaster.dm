...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard ICC-licensed newsfeed handler for use in commercial space stations. All the news you absolutely have no use for, in one place!"
	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			if(0)
				...
				dat += "<BR><FONT SIZE=1>Property of Horizons Co.</FONT>"
				if(GLOB.news_network.wanted_issue.active)
				...
				if(securityCaster)
					...
					dat+="<BR><A href='?src=[REF(src)];menu_censor_channel=1'>Mark Feed Channel with D-Notice</A>"
			...
			if(10)
				dat+="<B>News Feed Censorship Tool</B><BR>"
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
				...
				if(choice=="Confirm")
					...
					if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one.
						...
						if(GLOB.news_network.wanted_issue.isAdminMsg)
							alert("The wanted issue has been distributed by Head Office. You cannot edit it.","Ok")
							return
			...
			if(GLOB.news_network.wanted_issue.isAdminMsg)
				alert("The wanted issue has been distributed by a Head Office. You cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a Head Office. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by a Head Office. You cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by Head Office. You cannot place a D-Notice upon it.","Ok")
				return
...
/obj/item/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard most low-end Space Stations."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>67% styrofoam newspaper, for companies who can't afford actual paper.</FONT></div><HR>"
