...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
			...
			if(GLOB.news_network.wanted_issue.isAdminMsg)
				alert("The wanted issue has been distributed by Head Office. You cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by Head Office. You cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by Head Office. You cannot censor its author.","Ok")
				return
