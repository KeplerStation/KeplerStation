...
/obj/item/paper/contract/employment/update_text()
	...
	info = "<center>Conditions of Employment</center><BR><BR><BR><BR>This Agreement is made and entered into as of the date of last signature below, by and between [target] and the ICC.<BR>WITNESSETH:<BR>WHEREAS, [target]] is a natural born human or humanoid, possessing skills upon which he can aid the ICC, who seeks employment with the ICC.<BR>WHEREAS, the ICC agrees to sporadically provide payment to [target], in exchange for indentured servitude.<BR>NOW THEREFORE in consideration of the mutual covenants herein contained, and other good and valuable consideration, the parties hereto mutually agree as follows:<BR>In exchange for payments, [target]] agrees to work for the ICC, until extant debts are paid.<BR>Signed,<BR><i>[target]</i>"
...
/obj/item/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(M.mind == target && !M.owns_soul())
		if(user.mind && (user.mind.assigned_role == "Lawyer"))
			...
			deconvert = prob (5)
	if(deconvert)
		M.visible_message("<span class='notice'>[user] reminds [M] that [M]'s soul was already purchased!</span>")
		to_chat(M, "<span class='boldnotice'>You feel that your soul has returned to its rightful owner.</span>")
		M.return_soul()
	else
