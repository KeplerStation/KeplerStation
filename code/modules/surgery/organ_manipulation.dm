...
/datum/surgery_step/manipulate_organs
	...
	var/implements_heal_robotic = list(/*/obj/item/stack/nanopaste = 100, *//obj/item/screwdriver = 30) // UNCOMMENT WHEN WE ADD IPCS
	var/current_type
...
/datum/surgery_step/manipulate_organs/New()
	..()
	implements = implements + implements_extract + implements_heal + implements_heal_robotic
...
/datum/surgery_step/manipulate_organs/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(current_type == "insert")
		...
		display_results(user, target, "<span class='notice'>You successfully mend the damage to [target]'s [english_list(healed_organs)].</span>",
				"[user] successfully mends the damage to [target]'s [english_list(healed_organs)]!",
				"[user] successfully mends the damage to [target]'s [english_list(healed_organs)]!") // Why are these lines repeated? Coderbus, I want answers.
