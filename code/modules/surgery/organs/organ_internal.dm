...
/obj/item/organ/proc/check_damage_thresholds(var/M)
	...
	if(delta > 0)
		...
		if(owner)
			owner.med_hud_set_status()
		else //Processing is stopped when the organ is dead and outside of someone. This hopefully should restart it if a removed organ is repaired outside of a body.
			START_PROCESSING(SSobj, src)
