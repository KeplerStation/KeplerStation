...
/datum/nanite_program/research/active_effect()	
	if(!iscarbon(host_mob))	
		return	
	var/points = NANITE_RESEARCH_CLIENT	
	if(!host_mob.client) //less brainpower	
		points *= NANITE_RESEARCH_NOCLIENT	
...
/datum/nanite_program/researchplus/enable_passive_effect()	
	...
	if(!iscarbon(host_mob))	
		return	
	if(host_mob.client)	
		SSnanites.neural_network_count += NANITE_RESEARCH_CLIENT	
	else	
		SSnanites.neural_network_count += NANITE_RESEARCH_NOCLIENT	
...
/datum/nanite_program/researchplus/active_effect()	
	if(!iscarbon(host_mob))	
		return	
	var/mob/living/carbon/C = host_mob	
	var/points = round(SSnanites.neural_network_count / NANITE_RESEARCH_DIVIDE, 0.1)	
