...
datum/bounty/reagent/simple_drink/New()
	...
	description = "Head Office is thirsty! Send a shipment of [name] to quench the company's thirst."
	reward += rand(0, 2) * 500
...
datum/bounty/reagent/complex_drink/New()
	...
	wanted_reagent = new reagent_type
	name = wanted_reagent.name
	description = "The ICC is offering a reward for talented mixologists. Ship a container of [name] to claim the prize."
	reward += rand(0, 4) * 500
...
datum/bounty/reagent/chemical/New()
	...
	wanted_reagent = new reagent_type
	name = wanted_reagent.name
	description = "The ICC is in desperate need of the chemical [name]. Ship a container of it to be rewarded."
	reward += rand(0, 4) * 500
