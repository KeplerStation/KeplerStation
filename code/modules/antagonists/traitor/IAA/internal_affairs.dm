...
/datum/antagonist/traitor/internal_affairs/proc/greet_iaa()
	...
	if(syndicate)
		...
		to_chat(owner.current, "<B><font size=5 color=red>Any damage you cause will be a further embarrassment to Horizons, so you have no limits on collateral damage.</font></B>")
		to_chat(owner.current, "<span class='userdanger'> You have been provided with a standard uplink to accomplish your task. </span>")
		to_chat(owner.current, "<span class='userdanger'>By no means reveal that you, or any other Horizons employees, are undercover agents.</span>")
	else
